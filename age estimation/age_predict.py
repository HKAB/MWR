...
def age_estimate_mwr(model, img_path, face_detect = face_detect, save_folder='result', img_size=224, imagenet_stats = imagenet_stats, m = torch.nn.Softmax(dim=1), age_range=torch.arange(21, 61).float()):
    face_img, image_viz, box = face_detect(img_path)
    if face_img is not None:
        face_img = Image.fromarray(face_img)
        img = face_img.resize((img_size, img_size))
