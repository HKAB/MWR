...
class ImageLoader_New(Dataset):
    def __init__(self, path_data, img_size=224, aug=False):
        ...
        for i in range(len(self.df)):
            img_path = Path('UTK_mtcnn/images', self.df["filename"].iloc[i])
            self.img.append(str(img_path))
            self.age.append(self.df['age'].iloc[i])
...
if __name__ == '__main__':
    ...
    train_path = 'datalist/utk/UTK_train_coral.csv'
    test_path = 'datalist/utk/UTK_test_coral.csv'

    # train_loader = ImageLoader_New(train_path, aug=True)
    test_loader = ImageLoader_New(test_path, aug=False)

    # train_loader = DataLoader(train_loader, batch_size=8, shuffle=True, num_workers=4)
    test_loader = DataLoader(test_loader, batch_size=4, shuffle=False, num_workers=4)
